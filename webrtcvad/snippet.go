...
// example from github.com/maxhawkins/go-webrtcvad

func main() {
	flag.Parse()
	if flag.NArg() < 1 {
		log.Fatal("please provide infile.wav")
	}
	...
	report := func() {
		...
		_, err := f.WriteString(fmt.Sprintf("isActive = %v, t = %v\n", isActive, t))
		if err != nil {
			log.Fatal(err)
		}
	}
